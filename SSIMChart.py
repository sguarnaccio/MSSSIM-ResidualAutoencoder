...
modelsSSIM = model_path + 'resConvMSSSIML1_rgb2' + '.h5'
model=load_model(modelsSSIM, custom_objects={'ResidualCalculation':ResidualCalculation, 'lossMSSSIML1_V3':lossMSSSIML1_V3, 'binarizationLayer':binarizationLayer, 'tf_binarization':tf_binarization, 'yuv_conversion':yuv_conversion, 'yuvToRgb_conversion':yuvToRgb_conversion, 'hsv_conversion':hsv_conversion, 'hsvToRgb_conversion':hsvToRgb_conversion})
print("Done Loading")
print("Predictions")
...
jpegNoChrom_SSIM = np.zeros((steRange,))
...
for i in range (1,steRange+1):
    ...
    if averageSSIM == True:
        ...
        for j in range(avgRange):
            ...
            #ssim_pred += getSSIM(X_test[keys[j]], recImg[keys[j]])
            #ssim_L2 += getSSIM(X_test[keys[j]], recImgL2[keys[j]])
            #ssim_L1 += getSSIM(X_test[keys[j]], recImgL1[keys[j]])
...
if loadDataFrame == True:
    df = pd.read_csv(state_path+'dataFrameFinal.csv', keep_default_na=False, na_values=[""])
...
df.to_csv(path_or_buf=state_path+'dataFrameFinal.csv', index=False)
